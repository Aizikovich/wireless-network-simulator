...
class NRBaseStation:
    ...
    def __init__(self, bs_id, total_prb, prb_bandwidth_size, number_subcarriers, numerology, antenna_power,
                 antenna_gain, feeder_loss, carrier_frequency, total_bitrate, position, env):
        ...
        if (carrier_frequency < 150 or carrier_frequency > 2000):
            raise Exception(
                "your results may be incorrect because your carrier frequency is outside the boundaries of COST-HATA path loss model")
    ...
    def compute_nprb_NR(self, data_rate, rsrp):
        ...
        for elem in rsrp:
            if elem != self.bs_id and util.find_bs_by_id(elem).bs_type != "sat" and util.find_bs_by_id(
                ...
                interference = interference + (10 ** (rsrp[elem] / 10)) * (used / total) * (
                            self.allocated_prb / self.total_prb)
    ...
    def new_state(self):
        return (sum(self.resource_utilization_array) - self.resource_utilization_array[
            self.resource_utilization_counter] + self.allocated_prb) / (self.total_prb * self.T)
