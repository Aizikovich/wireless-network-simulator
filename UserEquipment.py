...
class user_equipment:
    ...
    def connect_to_bs_random(self):
        ...
        print(f"[CONNECTION_ESTABLISHED]: "
              f"UE{self.ue_id} connect to BS{bs} with data rate of {data_rate:2f}/{self.requested_bitrate} Mbps")
        # print("[CONNECTION_ESTABLISHED]: User ID %s is now connected to base_station %s with a data rate of %s/%s Mbps" % (
        #     self.ue_id, bs, data_rate, self.requested_bitrate))
    ...
    def connect_to_bs_id(self, bs_id):
        ...
        if bs_id not in available_bs:
            ...
            if bs_id not in self.bs_bitrate_allocation:
                ...
                return
            data_rate = util.find_bs_by_id(bs_id).request_connection(self.ue_id, self.bs_bitrate_allocation[bs_id],
                                                                     available_bs)
        ...
        for elem in curr_bs:
            if elem in available_bs:
                if self.current_bs[elem] == 0:
                    ...
                    continue
                data_rate = util.find_bs_by_id(elem).update_connection(self.ue_id, self.bs_bitrate_allocation[elem],
                                                                       available_bs)
    ...
    def initial_timestep(self):
        ...
        self.wardrop_sigma = (self.env.wardrop_epsilon) / (
                    2 * self.env.sampling_time * self.env.wardrop_beta * self.requested_bitrate * (len(rsrp) - 1) * len(
                self.env.ue_list))
    ...
    def next_timestep(self):
        ...
        bs_to_remove = [b for b in self.bs_bitrate_allocation if b not in rsrp]
        for elem in bs_to_remove:
        ...
        for p in self.bs_bitrate_allocation:
            for q in self.bs_bitrate_allocation:
                if p != q:
                    ...
                    mu_pq = 1
                    if (l_p - l_q) < self.env.wardrop_epsilon or bs_q.allocated_bitrate >= bs_q.total_bitrate - (
                            self.env.wardrop_epsilon / (2 * self.env.wardrop_beta)):
                        mu_pq = 0

                    mu_qp = 1
                    if (l_q - l_p) < self.env.wardrop_epsilon or bs_p.allocated_bitrate >= bs_p.total_bitrate - (
                            self.env.wardrop_epsilon / (2 * self.env.wardrop_beta)):
                        mu_qp = 0
