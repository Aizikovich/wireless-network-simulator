...
class wireless_environment:
    ...
    def insert_ue(self, ue_class, starting_position=None, speed=0, direction=0):
        if starting_position is not None and (starting_position[2] > 10 or starting_position[2] < 1):
            ...
            raise Exception(
                "Invalid service class for the UE, available service classes are: %s" % (ue.ue_class.keys()))
        ue_id = -1
        ...
        if starting_position is None:
            new_ue = ue.user_equipment(ue.ue_class[ue_class], ue_class, ue_id,
                                       (random.randint(0, self.x_limit), random.randint(0, self.y_limit), 1), self,
                                       speed * self.sampling_time, direction)
        else:
            new_ue = ue.user_equipment(ue.ue_class[ue_class], ue_class, ue_id, starting_position, self,
                                       speed * self.sampling_time, direction)
    ...
    def place_LTE_base_station(self, position, carrier_frequency, antenna_power, antenna_gain, feeder_loss,
                               available_bandwidth, total_bitrate):
        ...
        if (available_bandwidth in LTEbs.LTEbandwidth_prb_lookup):
            ...
            new_bs = LTEbs.LTEBaseStation(len(self.bs_list), LTEbs.LTEbandwidth_prb_lookup[available_bandwidth] * 10,
                                          180, 12, antenna_power, antenna_gain, feeder_loss, carrier_frequency,
                                          total_bitrate, position, self)
        else:
    ...
    def place_NR_base_station(self, position, carrier_frequency, numerology, antenna_power, antenna_gain, feeder_loss,
                              available_bandwidth, total_bitrate, drone=False):
        ...
        if available_bandwidth in NRbs.NRbandwidth_prb_lookup[numerology][fr]:
            prb_size = 15 * (
                        2 ** numerology) * 12  # 15KHz*12subcarriers for numerology 0, 30KHz*12subcarriers for numerology 1, etc.
            ...
            #  so we have to multiply by the number of time slots (sub-frames in LTE terminology) in a time frame
            if drone == False:
                new_bs = NRbs.NRBaseStation(len(self.bs_list),
                                            NRbs.NRbandwidth_prb_lookup[numerology][fr][available_bandwidth] * (
                                                        10 * 2 ** numerology), prb_size, 12, numerology, antenna_power,
                                            antenna_gain, feeder_loss, carrier_frequency, total_bitrate, position, self)
            else:
                new_bs = DRONEbs.DroneBaseStation(len(self.bs_list), DRONEbs.NRbandwidth_prb_lookup[numerology][fr][
                    available_bandwidth] * (10 * 2 ** numerology), prb_size, 12, numerology, antenna_power,
                                                  antenna_gain, feeder_loss, carrier_frequency, total_bitrate, position,
                                                  self)
        else:
            raise Exception(
    ...
    def place_DRONE_relay(self, starting_position, linked_bs_id, carrier_frequency, amplification_factor, antenna_gain,
                          feeder_loss):
        new_bs = DRONEbs.DroneRelay(len(self.bs_list), linked_bs_id, amplification_factor, antenna_gain, feeder_loss,
                                    carrier_frequency, starting_position, self)
        self.bs_list.append(new_bs)
        return new_bs.bs_id

    def place_DRONE_base_station(self, position, carrier_frequency, numerology, antenna_power, antenna_gain,
                                 feeder_loss, available_bandwidth, total_bitrate):
        return self.place_NR_base_station(position, carrier_frequency, numerology, antenna_power, antenna_gain,
                                          feeder_loss, available_bandwidth, total_bitrate, drone=True)
    ...
    def next_timestep(self):
        ...
        if self.wardrop_epsilon > self.wardrop_beta * ue.ue_class[0] * len(self.ue_list):
            print("Warning: Epsilon is outside the admissible ranges (", self.wardrop_epsilon, "/",
                  self.wardrop_beta * ue.ue_class[0] * len(self.ue_list), ")")
